...
int background_proc_copy[MAX_BACKGROUND_PROCESSES];
int background_proc_ptr_copy = 0;
extern char hd[MAX_PATH_SIZE];
extern int curr_pid;

// I have used Copilot in autocompletion
...
int system_commands(int fore_background, char **token_array, int size_array)
{
    ...
    if (pid == 0)
    {
        ...
        signal(SIGINT, SIG_DFL); // allows signals to work in child process
        signal(SIGTSTP, SIG_DFL); // allows signals to work in child process
        signal(SIGTTOU, SIG_DFL); // allows signals to work in child process
    ...
    else if (pid > 0)
    {
        ...
        if (fore_background == 0)
        {
            curr_pid = pid;
            // foreground process
            pid_t ans = waitpid(curr_pid, NULL, WUNTRACED);

            if(WIFSTOPPED(status))
            {
                // ctrl z pressed
                printf("Process sent to background with pid %d and stopped\n", pid); // add to the background process array
                background_proc[background_proc_ptr] = pid;
                background_proc_ptr++;
                background_proc_copy[background_proc_ptr_copy] = pid;
                background_proc_ptr_copy++;

                kill(pid, SIGTSTP); // send the correct signal to the process
